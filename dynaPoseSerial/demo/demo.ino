...
int servoError = 0 ; // 0 = no errors, continue with the program. 1 = error on at least one servo 
                     //(this usually means a servo cannot be found)
int isTorqueOn = 0;
int nextSavePose;
...
void loop() 
{
   if (Serial.available() > 0) 
   {
     ...
     temp = Serial.readBytes(buffer,2);
...
void checkServos()
{
  ...
  for(int i = 0; i<SERVO_COUNT;i++)
  {
    int id = i+1;  //the ids for the servos start at 1 while the array indexes start at 0. 
                   //Add '1' to the array index to get the two to match up
